...
// Source: https://github.com/blubs/NDK_Engine/blob/master/app/src/main/jni/engine/common.hpp
...
struct Vec3 {
    ...
    // TODO - Fix this for Quake coordinate system
	static Vec3 FRONT() {
		Vec3 result;
		result.y = 1.0f;
		return result;
...
// The mapping between index and matrix element:
// [ 0  3  6 ]
// [ 1  4  7 ]
// [ 2  5  8 ]
struct Mat3x3 {
	...
	Mat3x3(const float a0,const float a1,const float a2,
	...
	friend Mat3x3 operator*(const Mat3x3& l, const Mat3x3& r) {
		Mat3x3 result;
	...
	friend Vec3 operator*(const Vec3& l,const Mat3x3& r) {
		Vec3 result;
	...
	friend Vec3 operator*(const Mat3x3& l,const Vec3& r) {
		Vec3 result;
	...
	static Mat3x3 ROTATE(const Quat& q) {
		Mat3x3 result;
    ...
    // This does not conserve volume
	static Mat3x3 LERP(const Mat3x3 a, const Mat3x3 b, const float t) {
		Mat3x3 result;
...
struct Mat4x4 {
	...
	Mat4x4(const float a0,const float a1,const float a2,const float a3,
	...
	friend Mat4x4 operator*(const Mat4x4& l, const Mat4x4& r) {
		Mat4x4 result;
	...
	friend Vec3 operator*(const Vec3& l,const Mat4x4& r) {
		Vec3 result;
	...
	// if w = 1, full transformation, if w = 0, only rotate the point
	friend Vec3 operator*(const Mat4x4& l,const Vec3& r) {
		Vec3 result;
	...
	// This assumes the matrix represents a rigid body transformation
	...
	// Tranposes the matrix in-place
	void transpose() {
		Mat4x4 original = *this;
		//0 is unmodified
		m[1] = original.m[4];
		m[2] = original.m[8];
		m[3] = original.m[12];
	...
	Mat4x4 inverted_then_transposed() const {
		Mat4x4 result = inverted();
		result.transpose();
		return result;
	}
	...
	static Mat4x4 SCALE(const float x,const float y,const float z) {
		Mat4x4 result;
		result.m[0] = x;
		result.m[5] = y;
		result.m[10] = z;
	...
	static Mat4x4 ROT_TRANS(const Quat& rot, const Vec3& pos) {
		return TRANSLATE(pos)*ROTATE(rot);
	}

	// Alternate ROT_TRANS, assigns forward right and up vectors that are passed in
	static Mat4x4 ROT_TRANS(const Quat& rot, const Vec3& pos, Vec3* right, Vec3* up, Vec3* forward) {
		Mat4x4 rot_mat = ROTATE(rot);
	...
	static Mat4x4 ROT_TRANS(const Vec3& angles, const Vec3& pos) {
		...
		return Mat4x4::TRANSLATE(pos) * Mat4x4::ROTATE(rot);
	}

	// Alternate ROT_TRANS, assigns forward right and up vectors that are passed in
	static Mat4x4 ROT_TRANS(const Vec3& angles, const Vec3& pos, Vec3* right, Vec3* up, Vec3* forward) {
		Quat pitch(angles.x, Vec3::RIGHT());
		// Rotating angles.y about UP vector for yaw
		Quat yaw(angles.y, Vec3::UP());
	...
	static Mat4x4 VIEW(const Vec3 right, const Vec3 up, const Vec3 forward, const Vec3 pos) {
		Mat4x4 result;
	...
	static Mat4x4 PROJECT_PERSPECTIVE(const float near,const float far,const float aspect,const float fov) {
		Mat4x4 result;
	...
	// NOTE - may or may not change this, but currently ortho projection considers the center to be at (0,0)
	static Mat4x4 PROJECT_ORTHO(const float near, const float far, const float width, const float height) {
	...
	static Mat4x4 PROJECT_INFINITE(const float near,const float aspect,const float fov) {
		Mat4x4 result;
		float inv_tan_fov = 1.0f / (tanf(fov * 0.5f));
	...
	static Mat4x4 LERP(const Mat4x4 a, const Mat4x4 b, const float t) {
		Mat4x4 result;
		result.m[0] =  a.m[0]  + t*(b.m[0]  - a.m[0]);
		result.m[1] =  a.m[1]  + t*(b.m[1]  - a.m[1]);
		result.m[2] =  a.m[2]  + t*(b.m[2]  - a.m[2]);
	...
	//Eigen vectors / values (doubt I'll need these)
};
