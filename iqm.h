...
mesh_t *load_iqm_file(const char*file_path) {
    ...
    mesh->n_verts = iqm_header->n_verts;
    mesh->verts = (vertex_t*) malloc(sizeof(vertex_t) * mesh->n_verts);
    mesh->n_submeshes = iqm_header->n_meshes;
    mesh->submeshes = (submesh_t*) malloc(sizeof(submesh_t) * mesh->n_submeshes);

    vec3_t *verts_pos = (vec3_t*) malloc(sizeof(vec3_t) * mesh->n_verts);
    vec2_t *verts_uv = (vec2_t*) malloc(sizeof(vec2_t) * mesh->n_verts);
    ...
    iqm_parse_float_array(iqm_data, iqm_verts_pos, (float*) verts_pos, mesh->n_verts, 3, (float*) &default_vert);
    iqm_parse_float_array(iqm_data, iqm_verts_uv, (float*) verts_uv, mesh->n_verts, 2, (float*) &default_uv);

    // Populate verts array:
    for(unsigned int i = 0; i < mesh->n_verts; i++) {
        mesh->verts[i].x = verts_pos[i].pos[0]; 
        mesh->verts[i].y = verts_pos[i].pos[1];
        mesh->verts[i].z = verts_pos[i].pos[2];
        mesh->verts[i].u = verts_uv[i].pos[0];
        mesh->verts[i].v = verts_uv[i].pos[1];
        mesh->verts[i].color = 0xffffffff;
    }
    // ------------------------------------------------------------------------
    ...
    for(unsigned int i = 0; i < iqm_header->n_meshes; i++) {
        ...
        mesh->submeshes[i].n_tri_verts = n_tris * 3;
        mesh->submeshes[i].tri_verts = (unsigned int*) malloc(sizeof(unsigned int) * n_tris * 3);


        for(unsigned int j = 0; j < iqm_header->n_tris; j++) {
            unsigned int vert_a = ((iqm_tri_t*)(iqm_data + iqm_header->ofs_tris) + first_tri)[j].vert_idxs[0] - first_vert;
            unsigned int vert_b = ((iqm_tri_t*)(iqm_data + iqm_header->ofs_tris) + first_tri)[j].vert_idxs[1] - first_vert;
            unsigned int vert_c = ((iqm_tri_t*)(iqm_data + iqm_header->ofs_tris) + first_tri)[j].vert_idxs[2] - first_vert;
            mesh->submeshes[i].tri_verts[i*3 + 0] = vert_a;
            mesh->submeshes[i].tri_verts[i*3 + 1] = vert_b;
            mesh->submeshes[i].tri_verts[i*3 + 2] = vert_c;
        }
        mesh->submeshes[i].n_verts = n_verts;
        mesh->submeshes[i].n_tris = n_tris;
    }
